...
{
        internal class MatrixGraph : ICloneable
    {
        ...
        public object Clone()
        {
            var mGraph = (MatrixGraph)MemberwiseClone();
            mGraph.graph = new List<List<Vertex>>();
            foreach(var l in this.graph) {
                var list = new List<Vertex>();
                foreach(var v in l) {
                    list.Add((Vertex)v.Clone());
                }
                mGraph.graph.Add(list);
            }
            return mGraph;
        }

        public MatrixGraph(MatrixGraph graph)
        {
            this.directed = graph.directed;
            this.pondered = graph.pondered;
        ...
        public MatrixGraph readFile(string path)
        {
            ...
            for (int i = 1; i < lines.Count(); i++)
            {
                ...
                if (info.Count() == 3) {
                    if (info[2][0] == '.') {
                        value = Double.Parse("0," + info[2].Split('.')[1]);
                    } else {
                        value = Double.Parse(info[2]);
                    }
                }
                
                ret.addLink(info[0], info[1], value);
            }
            return ret;
        }
    }
